...
Function Main()
  ...
  DEFINE WINDOW oWnd TITLE "T-Gtk TreeStore power!!" SIZE 600,400
   DEFINE BOX oBox VERTICAL  SPACING 8 OF oWnd
     ...
     DEFINE TREEVIEWCOLUMN oCol COLUMN 3  TYPE "progress" OF oCol EXPAND
...
STATIC FUNCTION Create_Model()
  ...
  local nToday  := 0
  local aDays   := { "Lunes", "Martes", "Miércoles", "Jueves", ;
...
//   pixbuf := gdk_pixbuf_new_from_file( "../../images/gnome-gsame.png" )
  ...
  for nMonth := 1 to len( aMonths )
      ...
      SET TREE_STORE oLbx ITER aParent POS 3 VALUE 50
...
//             SET TREE_STORE oLbx ITER aChild POS 3 VALUE pixbuf
             SET TREE_STORE oLbx ITER aChild POS 3 VALUE 100
          else
             SET TREE_STORE oLbx ITER aChild POS 3 VALUE nDay
          endif
      next
  next
...
//  gdk_pixbuf_unref( pixbuf )
  ...
  SET TREE_STORE oLbx ITER aParent POS 3 VALUE 75     // Valor de Progress
  
      INSERT TREE_STORE oLbx ROW 1 ;
             ITER aChild PARENT aParent ;
             VALUES "Hijo",.T.,100 // Hijo con valores directamente
