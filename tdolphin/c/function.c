...
#define BIND_TYPE        1
#define BIND_IS_NULL     2
#define BIND_IS_UNSIGNED 3
#define BIND_LENGTH      4

#ifdef _WIN_
LPSTR LToStr( double w )
{
   static char dbl[ HB_MAX_DOUBLE_LENGTH + 10 ];
...
static HB_GARBAGE_FUNC( MYSQL_STMT_release )
{
   void ** ph = ( void ** ) Cargo;
   ...
   {
      /* Destroy the object */
      mysql_stmt_close( ( MYSQL_STMT * ) * ph );
...
static const HB_GC_FUNCS s_gcMYSQL_STMTFuncs =
{
   MYSQL_STMT_release,
   hb_gcDummyMark
};
...
static void hb_MYSQL_STMT_ret( MYSQL_STMT * p )
{
   if( p )
   {
#ifndef __XHARBOUR__
#if HB_PCODE_VER > 2
      void ** ph = ( void ** ) hb_gcAllocate( sizeof( MYSQL_STMT * ), &s_gcMYSQL_STMTFuncs );
#else
      void ** ph = ( void ** ) hb_gcAllocate( sizeof( MYSQL_STMT * ), MYSQL_STMT_release );
#endif      
#else
      void ** ph = ( void ** ) hb_gcAlloc( sizeof( MYSQL_STMT * ), MYSQL_STMT_release );
#endif //__XHARBOUR__
      * ph = p;
...
static MYSQL_STMT * hb_MYSQL_STMT_par( int iParam )
{
#ifndef __XHARBOUR__      
#if HB_PCODE_VER > 2  
   void ** ph = ( void ** ) hb_parptrGC( &s_gcMYSQL_STMTFuncs, iParam );
#else
   void ** ph = ( void ** ) hb_parptrGC( MYSQL_STMT_release, iParam );
#endif
#else
   void ** ph = ( void ** ) hb_parptrGC( MYSQL_STMT_release, iParam );
#endif //__XHARBOUR__

   return ph ? ( MYSQL_STMT * ) * ph : NULL;
}
...
HB_FUNC( MYSTMTINIT )
{
   ...
   if( hMySql )
   {
      stmt = mysql_stmt_init( hMySql );
   }else 
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );	
   
   hb_MYSQL_STMT_ret( stmt );
}
...
HB_FUNC( MYSTMTCLOSE )
{
   MYSQL_STMT * stmt = ( MYSQL_STMT * ) hb_MYSQL_STMT_par( 1 );
...
HB_FUNC( MYSTMTERRNO )
{
   ...
   if( stmt )
   {
      hb_parni( mysql_stmt_errno( stmt ) );
   }else 
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );		
}
...
HB_FUNC( MYSTMTSQLSTATE )
{
   MYSQL_STMT * stmt = ( MYSQL_STMT * ) hb_MYSQL_STMT_par( 1 );
   
   if( stmt )
   {
      hb_retc( mysql_stmt_sqlstate( stmt ) );
   }else 
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );		
}
...
HB_FUNC( MYSTMTPREPARE )
{
   MYSQL_STMT * stmt = ( MYSQL_STMT * ) hb_MYSQL_STMT_par( 1 );
   
   if( stmt )
   {
      hb_retni( mysql_stmt_prepare( stmt, hb_parc( 2 ), hb_parclen( 2 ) ) ); 
   }else 
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );		
}
...
HB_FUNC( MYSTMTBINDPARAM )
{
   MYSQL_STMT * stmt = ( MYSQL_STMT * ) hb_MYSQL_STMT_par( 1 );
   MYSQL_BIND * bind = ( MYSQL_BIND * ) hb_parptr( 2 ); 
   
   if( stmt )
   {
      hb_retni( mysql_stmt_bind_param( stmt, bind )); 
   }else 
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );		
}

//------------------------------------------------//
HB_FUNC( MYSTMTCOUNTPARAM )
{
	 int param_count;
   MYSQL_STMT * stmt = ( MYSQL_STMT * ) hb_MYSQL_STMT_par( 1 );
...
HB_FUNC( MYSTMTEXECUTE )
{
   MYSQL_STMT * stmt = ( MYSQL_STMT * ) hb_MYSQL_STMT_par( 1 );
   
   if( stmt )
   {
      hb_retni( mysql_stmt_execute( stmt ) ); 
   }else 
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );		
}
...
unsigned int InternalSeek( MYSQL_RES* presult, int iData, unsigned int uiField, BOOL bSoft, char * cSearch )
{
   ...
   hb_strncpy( ctempSearch, cSearch, iLen );
      ...
      while( uiStart < uiEnd )
      {
        ...
        if( i > 0 ){
            ...
            { 
                 ...
                 ctempSearch = ( char * ) hb_xgrab( sizeof( char * ) * lencSearch );
                 hb_strncpy( ctempSearch, cSearch, lencSearch );
                 if( bSoft )
                 {
                    hb_strncpy( cSrc, row[ lField ], lencSearch );
                    hb_strLower( cSrc, lencSearch  );
                    hb_strLower( ctempSearch, lencSearch );
                 }
                 else
            	       hb_strncpy( cSrc, row[ lField ], lencSearch );
...
HB_FUNC( GETBIND )
{
   ...
   switch( bind->buffer_type )
   {
      ...
      case MYSQL_TYPE_INT24:    
          hb_retnl( * (( long * ) bind->buffer) );
          break;
      case MYSQL_TYPE_LONG:    
      case MYSQL_TYPE_YEAR:
      case MYSQL_TYPE_SHORT:
          hb_retni( * (( int * ) bind->buffer) );
          break;  
      case MYSQL_TYPE_DECIMAL:   
      case MYSQL_TYPE_FLOAT:
      case MYSQL_TYPE_DOUBLE:
      	  hb_retnd( * (( double * ) bind->buffer) );
      	  break;          
   }
...
//------------------------------------------------//
// MYSQL_BIND handle, Position, Item, Property, Property value
HB_FUNC( SETBIND )
{
   PHB_ITEM pData;
   MYSQL_BIND * pBind;
   MYSQL_BIND * bind;
   int iType = hb_parni( 4 );
   ...
   {
      case BIND_TYPE:
      	bind->buffer_type = hb_parni( 5 );
      	switch( bind->buffer_type )
      	{
      		 case MYSQL_TYPE_LONGLONG:
           case MYSQL_TYPE_INT24:      		
              bind->buffer      = ( char * ) &pData->item.asLong.value;
              bind->is_unsigned = 0;
              bind->is_null     = 0;
              break;
      	   case MYSQL_TYPE_LONG:              
           case MYSQL_TYPE_YEAR:
      	   case MYSQL_TYPE_SHORT:
              bind->buffer      = ( char * ) &pData->item.asInteger.value;
              bind->is_unsigned = 0;
              bind->is_null     = 0;
              break;  
           case MYSQL_TYPE_DECIMAL:   
           case MYSQL_TYPE_FLOAT:
           case MYSQL_TYPE_DOUBLE:   
              bind->buffer      = ( char * ) &pData->item.asDouble.value;
              bind->is_unsigned = 0;
              bind->is_null     = 0;  
              break;         	   
        }
