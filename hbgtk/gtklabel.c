...
{
   GtkWidget * label = gtk_label_new_with_mnemonic( hb_parc( 1 ) );
   hb_retnl( ( glong) label );
}

HB_FUNC( GTK_LABEL_SET_TEXT ) //widget, cText
{
   GtkWidget * label = GTK_WIDGET( hb_parnl( 1 ) );
...
{
   GtkWidget * label = GTK_WIDGET( hb_parnl( 1 ) );
   gtk_label_set_markup_with_mnemonic( GTK_LABEL( label ), (gchar *)hb_parc( 2 ) );
}
...
{
   GtkWidget * label = GTK_WIDGET( hb_parnl( 1 ) );
   gtk_label_set_text_with_mnemonic( GTK_LABEL( label ), hb_parc( 2 ) );
}
...
{
   GtkWidget * label = GTK_WIDGET( hb_parnl( 1 ) );
   gtk_label_set_label( GTK_LABEL( label ), hb_parc(2) );
}
