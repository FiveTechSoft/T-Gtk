...
HB_FUNC( GTK_NOTEBOOK_NEW )
{
   ...
   hb_retnl( (ULONG) notebook );
}

HB_FUNC( GTK_NOTEBOOK_APPEND_PAGE )
{
   GtkWidget * notebook = ( GtkWidget * ) hb_parnl( 1 );
   GtkWidget * child = ( GtkWidget * ) hb_parnl( 2 );
   GtkWidget * label = ( GtkWidget * ) hb_parnl( 3 );
...
{
   GtkNotebook *notebook  = GTK_NOTEBOOK(  hb_parnl( 1 )  );
...
{
   gtk_notebook_next_page( GTK_NOTEBOOK( ( GtkWidget * ) hb_parnl( 1 ) ) );
}

HB_FUNC( GTK_NOTEBOOK_PREV_PAGE ) // widget
{
   gtk_notebook_prev_page( GTK_NOTEBOOK( ( GtkWidget * ) hb_parnl( 1 ) ) );
}
