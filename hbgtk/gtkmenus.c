...
{
  hb_retptr( ( GtkWidget * ) gtk_menu_new() );
}
...
{
   GtkWidget * menu_item = GTK_WIDGET( hb_parptr( 1 ) );
   GtkWidget * submenu   = GTK_WIDGET( hb_parptr( 2 ) );
...
{
   GtkMenuShell * menu_shell = GTK_MENU_SHELL( hb_parptr( 1 ) );
...
{
   GtkWidget * menuitem = GTK_WIDGET( hb_parptr( 1 ) );
   GtkWidget * image    = GTK_WIDGET( hb_parptr( 2 ) );
...
{
   GtkWidget * radio = gtk_radio_menu_item_new ( ( GSList * ) hb_parptr( 1 )  );
