...
{
  gtk_widget_show_all( GTK_WIDGET( hb_parnl( 1 ) ) );
}
...
{
  gtk_widget_destroy( GTK_WIDGET( hb_parnl( 1 )  ) );
}
...
{
   gtk_widget_grab_focus( ( GTK_WIDGET( hb_parnl( 1 ) ) ) );
}

HB_FUNC( GTK_WIDGET_SET_UPOSITION )
{
   gtk_widget_set_uposition( GTK_WIDGET( hb_parnl( 1 ) ),
                             hb_parnl( 3 ), hb_parnl( 2 ) );
}

HB_FUNC( GTK_WIDGET_SET_SENSITIVE ) // widget, boolean -> void
{
   gtk_widget_set_sensitive( GTK_WIDGET( hb_parnl( 1 ) ),
                             (gboolean) hb_parl( 2 ) );
}

HB_FUNC( GTK_WIDGET_MODIFY_FONT ) // widget, Font
{
   gtk_widget_modify_font( GTK_WIDGET( hb_parnl( 1 ) ),
                          ( PangoFontDescription * ) hb_parnl( 2 ) );
}
...
{
   GtkWidget * widget = GTK_WIDGET( hb_parnl( 1 ) );
   hb_retnl( (glong) gtk_widget_get_toplevel( widget ) );
}
...
{
   GtkWidget *widget = GTK_WIDGET( hb_parnl( 1 ) );
   gchar * accel_signal = hb_parc( 2 );
   GtkAccelGroup * accel_group = GTK_ACCEL_GROUP( hb_parnl( 3 ) );
