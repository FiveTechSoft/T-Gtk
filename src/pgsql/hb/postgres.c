/*
 * $Id: postgres.c 15157 2010-07-21 12:46:54Z vszakats $
 */
...
HB_FUNC( PQFINISH )
{
   ...
   /* Check if point is not NULL to avoid multiple freeing */
   if( ph && *ph )
   {
      /* Destroy the object */
      PQfinish( ( PGconn * ) *ph );
...
HB_FUNC( PQRESSTATUS )
{
    ExecStatusType resultstat = hb_parni( 1 );

    if( resultstat )
      hb_retc((char *) PQresStatus( resultstat ));
    else
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );
}
