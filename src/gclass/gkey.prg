...
METHOD New( oObj, oFont ) CLASS gKey
local x
  ::oObject := oObj
  ::oFont   := oFont
  ::aKey := {}
  ...
  if ValType( ::oObject ) = "A"
    aEval( ::oObject, { | a | gtk_signal_connect( a:pWidget, "key-press-event", { |oSender, pGdkEventKey| ::OnKey( oSender, pGdkEventKey ) } ) } )
  else
    gtk_signal_connect( ::oObject:pWidget, "key-press-event", { |oSender, pGdkEventKey| ::OnKey( oSender, pGdkEventKey ) } )
  end
RETURN self

METHOD OnKey( oSender, pGdkEventKey ) CLASS gKey
...
METHOD SetKey( xKey, bFun ) CLASS gKey
...
METHOD Display( oBox, cF1, cF2, cF3, cF4, cF5, cF6, cF7, cF8, cF9, cF10, cF11, cF12, lDobleLine ) CLASS gKey
  Local oBox1, oTable
  ...
  If lDobleLine
    ...
    DEFINE LABEL PROMPT cF11 EXPAND OF oTable TABLEATTACH 10,11,1,2
    DEFINE LABEL PROMPT " F12 " OF oTable TABLEATTACH 11,12,0,1
    DEFINE LABEL PROMPT cF12 EXPAND OF oTable TABLEATTACH 11,12,1,2
