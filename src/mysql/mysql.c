/*
 * $Id: mysql.c,v 1.1 2006-09-15 12:58:15 xthefull Exp $
 */
 ...
 *
 * Copyright 2006 Rafa Carmona <thefull@wanadoo.es>
 * Support Embebed Server
 * 
 * 
...
/* Dando soporte a servidor embebido 
 * (c) 2006 Rafa Carmona 
 */
...
int iEmbe;
    ...
    static const char *client_groups[] ={ "client", "embapp", NULL };
    static const char *server_groups[] ={ "server", "embedded", "embapp_server", NULL };
#endif
...
HB_FUNC(SQLCONNECT) // MYSQL *mysql_real_connect(MYSQL*, char * host, char * user, char * password, char * db, uint port, char *, uint flags)
{
   ...
   /*-----------------17/04/2006 18:26-----------------
    * from 4.0.x of MySql support Embedded Server.
    * --------------------------------------------------*/
   #if MYSQL_VERSION_ID >= 40000
       iEmbe = hb_parl( 6 ) ? TRUE : FALSE;
       if( iEmbe )
       {  
...
{
   mysql_close((MYSQL *)_parnl(1));  // Desconectar del servidor
   
   #if MYSQL_VERSION_ID >= 40000
       if( iEmbe )
       { 
        mysql_server_end(); // Cerrar el servidor incrustado. 
       }
   #endif
   hb_ret();
}
