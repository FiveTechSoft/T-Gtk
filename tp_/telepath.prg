/*
 * $Id: telepath.prg,v 1.1 2009-02-12 22:24:33 riztan Exp $
 */
...
function tp_open( nPort, nInSize, nOutSize, nBaud, nData, cParity, nStop, cPortname )
   ...
   #ifdef __PLATFORM__Linux
   default cPortName to "/dev/ttyS"    // /dev/ttyUSB  (Cable USB-Serial)
   #else
...
function te_errstr( nErrorCode )

   Local cErrorCode
         
   switch (nErrorCode)
      case TE_PARAM
         cErrorCode := "Bad parameter"
         exit
      
      case TE_NOPORT
         cErrorCode := "No such port"
         exit

      case TE_CLOSED
         cErrorCode := "Port not open"
         exit

      case TE_CONFL
         cErrorCode := "IRQ conflict"
         exit
      ...
      case TE_ESCAPE
         cErrorCode := "User escape"
         exit

      case TE_LENGTH
         cErrorCode := "Length limit"
         exit

      case TE_CANCEL
         cErrorCode := "Input canceled"
         exit

      case TE_NOHDL
         cErrorCode := "Out of handles"
         exit

      case TE_UCANCEL
         cErrorCode := "Canceled by user"
         exit

      case TE_RCANCEL
         cErrorCode := "Canceled by remote"
         exit

      case TE_STARTTM
         cErrorCode := "Timeout waiting to start"
         exit

      case TE_BLOCKTM
         cErrorCode := "Timeout waiting for block"
         exit

      case TE_ACKTM
         cErrorCode := "Timeout waiting for acknowledge"
         exit

      case TE_SENDTM
         cErrorCode := "Timeout waiting to send"
         exit

      case TE_CLEARTM
         cErrorCode := "Timeout waiting for sender to stop"
         exit

      case TE_NAK
         cErrorCode := "Negative acknowledge"
         exit

      case TE_BADACK
         cErrorCode := "Bad acknowledge character"
         exit

      case TE_BADBLK
         cErrorCode := "Bad block format"
         exit

      case TE_LONGBLK
         cErrorCode := "Long block received"
         exit

      case TE_ERRMAX
         cErrorCode := "Too many errors"
         exit

      case TE_DUPBLK
         cErrorCode := "Duplicate block"
         exit

      case TE_PROTO
         cErrorCode := "Protocol failure"
         exit

      case TE_CKSUM
         cErrorCode := "Checksum error"
         exit

      case TE_HDRTM
         cErrorCode := "Timeout waiting for Zmodem header"
         exit

      case TE_DISKFULL
         cErrorCode := "Disk full"
         exit

      case TE_NOFILE
         cErrorCode := "File not found"
         exit

      case TE_NOPATH
         cErrorCode := "Path not found"
         exit

      case TE_MFILE
         cErrorCode := "Too many open files"
         exit

      case TE_ACCESS
         cErrorCode := "Access denied"
         exit

      default
         cErrorCode := "Undefined Error"
         exit
